...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active">Alterar Paquete</li>
	</ol>
</section>
		...
		<form method='POST' class='atualstudio' action='?engine=pack_change&module=shop&pack=<?php echo $pack; ?>' enctype='multipart/form-data'>
			...
			<div class="box-body">
				...
				<div class='form-group'>
					...
					<p class='help-block' style='font-size: 13px;'>Caso insira um valor, o usuário poderá comprar todos os itens do Paquete duma vez só pelo valor informado. Deixe "0" para cancelar essa possibilidade.</p>
				</div>
				
				<div class='box-footer'>
					<input type='submit' class='btn btn-primary' value='Alterar Paquete' />
					<div style='float:right;'>
						<a href='javascript:history.back();' class='btn btn-default'>&laquo; Volver</a>
						<a href='?page=pack_delete&module=shop&pack=<?php echo $pack; ?>' title='Borrar' class='btn btn-danger'>Borrar</a>
					</div>
				</div>
	...
	<div class="box">
		...
		<div class="box-body">
			...
			<table class="table table-bordered item-list ui-sortable">
				...
				if(count($consulta) == 0) {
					echo'<tr><td colspan="8">Ningún item encontrado.</td></tr>';
				} else {
					for($i =0, $c=count($consulta);$i < $c; $i++) {
						echo"
						<tr id='item_".$consulta[$i]['sitem_id']."'>
							...
							<td>".($consulta[$i]['Acumulativo'] == '0' ? 'No' : 'Sí')."</td>
							<td>".date('d/m/Y \à\s H:i', ($consulta[$i]['data_c']))."</td>
							<td>".(empty($consulta[$i]['data_a']) ? 'Todavia no' : date('d/m/Y \à\s H:i', ($consulta[$i]['data_a'])))."</td>
							<td>
								<a class='btn btn-danger usarJquery' href='./?engine=item_delete&module=shop&pack=".$consulta[$i]['spack_id']."&item=".$consulta[$i]['sitem_id']."' title='Borrar' class='btn btn-danger'> <i class='fa fa-remove'></i></a>
							</td>
						</tr>
						";
					}
			...
			<div class='box-footer'>
				<a href='./?page=item_add&module=shop&pack=<?php echo $pack; ?>' class='btn btn-primary'>Anãdir item</a>
			</div>
