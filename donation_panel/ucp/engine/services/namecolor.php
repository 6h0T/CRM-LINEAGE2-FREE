...
$cached_op = pack("cccVV", 111, 0, 0, intval($cid), intval($hexColor));
$result = l2_cached_push(pack("s", strlen($cached_op)+2).$cached_op.ansi2unicode('site-atualstudio'));
if($result !== '1') {
	fim($LANG[12055].' #1');
}

$execute = Services::changeNameColor($_SESSION['acc'], $cid, $hexColor);
if($execute) {
	@Services::logServices($_SESSION['acc'], $cid, 'namecolor', vCode('Change nickname color to #'.$cor.'('.$hexColor.')'), $custo);
