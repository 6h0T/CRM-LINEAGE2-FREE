...
class Donate {
	...
	public static function findChar($acc, $personagem) {
		...
		$sql = DB::Executa("SELECT TOP 1 char_name, char_id AS obj_Id, CASE WHEN (login > logout OR logout IS NULL) AND login IS NOT NULL THEN 1 ELSE 0 END AS online FROM user_data WHERE account_name = '".$acc."' AND char_id = '".$personagem."'", "WORLD");
		return $sql;
