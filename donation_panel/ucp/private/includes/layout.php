...
<link rel="image_src" href="http://<?php echo $panel_url; ?>/imgs/image.jpg" />
...
<meta property="og:image" content="http://<?php echo $panel_url; ?>/imgs/image.jpg" />
	...
	?>
		...
		<div class='smallCenter'>
			...
			<form class='usarJquery' action='./?engine=login' method='POST'>
				...
				<input name='url' type='hidden' value='<?php echo urlencode("//".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']); ?>' />
		...
		<div class='grayBg'>
			...
			<aside>
				...
				<div class='saldo'>
					<div>
						...
						<div class='bts'>
							...
							if(!empty($funct['trnsf1']) || !empty($funct['trnsf2']) || !empty($funct['trnsf3'])) { echo "<a href='./?module=donate&page=transfer' class='default'>".$LANG[10013]."</a>"; }
							?>
						</div>
					</div>
				</div>
				...
				<nav>
					...
					<a href='../' target="_blank">
						<i class='fa fa-comment-o'></i>Official Forum
					</a>
					...
					<span<?php echo ($m == 'donate/' ? " class='actived'" : ""); ?>>
						...
						<div class='dropdown'>
							...
							if(!empty($funct['trnsf1']) || !empty($funct['trnsf2']) || !empty($funct['trnsf3'])) { echo "
					...
					if(!empty($funct['vote'])) {
						echo "
						<a href='./?module=vote&page=index'".($m == 'vote/' ? " class='actived'" : "").">
							<i class='fa fa-thumbs-up'></i>".$LANG[50001]."
						</a>
						";
					}
