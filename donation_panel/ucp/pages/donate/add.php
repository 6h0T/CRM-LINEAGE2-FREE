				...
				*/ ?>
				<td>
					<select style='min-width: 160px;' name='metodo_pgto' id='metodo_pgto'>
						...
						if(!empty($PayPal['actived'])) { echo "<option data-symbol='$' value='PayPal_USD'>PayPal Internacional ($3.30 USD = 100 Donation Coins)</option>"; }
						if(!empty($MercadoPago['actived'])) { echo "<option data-symbol='".$MercadoPago['symbol']."' value='MercadoPago'>MercadoPago ($30 ARS  = 1 Donation Coin)</option>"; }
						if(!empty($PagosOnline['actived'])) { echo "<option data-symbol='".$PagosOnline['symbol']."' value='PagosOnline'>PagosOnline ($5 ARS  = 1 Donation Coin)</option>"; }
						...
						if(!empty($PicPay['actived'])) { echo "<option data-symbol='".obtainCurrencySymbol($PicPay['currency'])."' value='PicPay'>PicPay</option>"; }
						if(!empty($Banking['actived'])) { echo "<option data-symbol='".obtainCurrencySymbol($Banking['currency'])."' value='Banking'>".$LANG[15003]."</option>"; }
                        if(!empty($Ualabis['actived'])) { echo "<option data-symbol='".obtainCurrencySymbol($Ualabis['currency'])."' value='Ualabis'>Ualabis ($30 ARS  = 1 Donation Coin)</option>"; }
                        if(!empty($Weear['actived'])) { echo "<option data-symbol='".obtainCurrencySymbol($Weear['currency'])."' value='Weear'>Wee.ar ($30 ARS  = 1 Donation Coin)</option>"; }
						?>
					</select>
				</td>
				...
				<td>
					<select style='min-width: 160px;' name='qtdCoins' id='qtdCoins'>
						...
						<?php
						$coinQntV = intval($coinQntV);
						for($i=(100*$coinQntV), $c=(500*$coinQntV); $i <= $c; $i++) { if($i%(100*$coinQntV)==0) { echo "<option value='".$i."'>".$i." ".$coinName."'s</option>"; } }
						for($i=(600*$coinQntV), $c=(1000*$coinQntV); $i <= $c; $i++) { if($i%(100*$coinQntV)==0) { echo "<option value='".$i."'>".$i." ".$coinName."'s</option>"; } }
						for($i=(1500*$coinQntV), $c=(5000*$coinQntV); $i <= $c; $i++) { if($i%(500*$coinQntV)==0) { echo "<option value='".$i."'>".$i." ".$coinName."'s</option>"; } }
						?>
					</select>
...
$(document).ready(function(){
	...
	$('select#metodo_pgto').change(function(){
		...
		var count3 = parseInt("<?php echo (isset($buyCoins['bonus_count'][3]) ? intval($buyCoins['bonus_count'][3]) : 1000); ?>");
		var bonus3 = parseInt("<?php echo (isset($buyCoins['bonus_percent'][3]) ? intval($buyCoins['bonus_percent'][3]) : 10); ?>");
